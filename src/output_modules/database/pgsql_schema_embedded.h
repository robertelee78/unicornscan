...
static const char *pgsql_schema_sequences_ddl =
	"CREATE SEQUENCE IF NOT EXISTS uni_scan_id_seq;\n"
...
static const char *pgsql_schema_scans_ddl =
	"CREATE TABLE IF NOT EXISTS uni_scan (\n"
	"    scan_id    BIGINT NOT NULL DEFAULT nextval('uni_scan_id_seq'),\n"
...
static const char *pgsql_schema_sworkunits_ddl =
	...
	"    CONSTRAINT uni_sworkunit_uniq_comp_LK UNIQUE (scan_id, wid)\n"
	");\n";
...
static const char *pgsql_schema_lworkunits_ddl =
	...
	"    wid         BIGINT NOT NULL,\n"
	"    status      SMALLINT NOT NULL,\n"
	"    CONSTRAINT uni_lworkunit_uniq_comp_LK UNIQUE (scan_id, wid)\n"
	");\n";
...
static const char *pgsql_schema_scan_phases_ddl =
	...
	"    CONSTRAINT uni_scan_phases_pk PRIMARY KEY (scan_id, phase_idx)\n"
	");\n";
...
static const char *pgsql_schema_indexes_ddl =
	...
	"CREATE INDEX IF NOT EXISTS uni_ipreport_scansid_idx ON uni_ipreport(scan_id);\n"
	"CREATE INDEX IF NOT EXISTS uni_arpreport_scansid_idx ON uni_arpreport(scan_id);\n"
	...
	"CREATE INDEX IF NOT EXISTS uni_scan_s_time_idx ON uni_scan(s_time);\n"
	/* Composite indexes for common query patterns */
	"CREATE INDEX IF NOT EXISTS uni_ipreport_scan_host_idx ON uni_ipreport(scan_id, host_addr);\n"
	"CREATE INDEX IF NOT EXISTS uni_ipreport_scan_dport_idx ON uni_ipreport(scan_id, dport);\n"
	/* GIN indexes for JSONB columns (efficient for containment queries) */
	"CREATE INDEX IF NOT EXISTS uni_scan_metadata_gin ON uni_scan USING gin(scan_metadata);\n"
...
static const char *pgsql_schema_constraints_ddl =
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_sworkunit_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_sworkunits ADD CONSTRAINT uni_sworkunit_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_lworkunit_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_lworkunits ADD CONSTRAINT uni_lworkunit_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_workunitstats_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_workunitstats ADD CONSTRAINT uni_workunitstats_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_output_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_output ADD CONSTRAINT uni_output_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_ipreport_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_ipreport ADD CONSTRAINT uni_ipreport_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_arpreport_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_arpreport ADD CONSTRAINT uni_arpreport_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	...
	"\n"
	"DO $$ BEGIN\n"
	"    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uni_scan_phases_uni_scan_fk') THEN\n"
	"        ALTER TABLE uni_scan_phases ADD CONSTRAINT uni_scan_phases_uni_scan_FK\n"
	"            FOREIGN KEY(scan_id) REFERENCES uni_scan(scan_id);\n"
	"    END IF;\n"
	"END $$;\n";
...
static const char *pgsql_schema_rls_policies_ddl =
	...
	"DROP POLICY IF EXISTS \"Allow full access to scans\" ON uni_scan;\n"
	...
	"CREATE POLICY \"Allow full access to scans\" ON uni_scan FOR ALL USING (true) WITH CHECK (true);\n"
...
static const char *pgsql_schema_migration_v2_ddl =
	/* Add JSONB columns */
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN scan_metadata JSONB DEFAULT '{}'::jsonb;\n"
 ...
 * Adds new columns to uni_scan for mode info, phases table, and user notes
 */
static const char *pgsql_schema_migration_v4_ddl =
	/* Add new columns to uni_scan for compound mode support */
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN mode_str VARCHAR(64);\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN mode_flags SMALLINT DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN num_phases SMALLINT DEFAULT 1;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN port_str TEXT;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN interface VARCHAR(64);\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN tcpflags INTEGER DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN send_opts INTEGER DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN recv_opts INTEGER DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN pps INTEGER DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN recv_timeout SMALLINT DEFAULT 0;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN repeats INTEGER DEFAULT 1;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	"    ALTER TABLE uni_scan ADD COLUMN scan_notes TEXT;\n"
	"EXCEPTION WHEN duplicate_column THEN NULL;\n"
	"END $$;\n"
	"\n"
...
static const char *pgsql_schema_migration_v5_ddl =
	...
	"CREATE INDEX IF NOT EXISTS uni_host_scans_scansid_idx ON uni_host_scans(scan_id);\n"
	"CREATE INDEX IF NOT EXISTS uni_hops_scansid_idx ON uni_hops(scan_id);\n"
	...
	"CREATE UNIQUE INDEX IF NOT EXISTS uni_services_uniq ON uni_services(host_addr, port, proto, scan_id);\n"
	...
	"CREATE INDEX IF NOT EXISTS uni_services_scansid_idx ON uni_services(scan_id);\n"
	"CREATE INDEX IF NOT EXISTS uni_osfingerprints_host_idx ON uni_os_fingerprints(host_addr);\n"
	"CREATE INDEX IF NOT EXISTS uni_osfingerprints_scansid_idx ON uni_os_fingerprints(scan_id);\n"
...
static const char *pgsql_schema_migration_v6_ddl =
	...
	"CREATE INDEX IF NOT EXISTS idx_geoip_scan ON uni_geoip(scan_id);\n"
...
/*
 * Schema v7 migration - add target_str and src_addr columns to uni_scan
 */
static const char *pgsql_schema_migration_v7_ddl =
	"ALTER TABLE uni_scan ADD COLUMN IF NOT EXISTS target_str TEXT;\n"
	"ALTER TABLE uni_scan ADD COLUMN IF NOT EXISTS src_addr INET;\n";
...
static const char *pgsql_schema_migration_v8_ddl =
	...
	"            FOREIGN KEY(first_scan_id) REFERENCES uni_scan(scan_id) ON DELETE SET NULL;\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
	"DO $$ BEGIN\n"
	...
	"            FOREIGN KEY(last_scan_id) REFERENCES uni_scan(scan_id) ON DELETE SET NULL;\n"
	"    END IF;\n"
	"END $$;\n"
	"\n"
