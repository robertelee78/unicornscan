...
*/
		...
		case 8:
			...
			if (s->ss->mode == MODE_TCPSCAN || s->ss->mode == MODE_TCPTRACE) {
				...
				s->ss->tcpoptions[4]=0x01;
				/* WS 8 (size 3) */
				s->ss->tcpoptions[5]=0x03; s->ss->tcpoptions[6]=0x03; s->ss->tcpoptions[7]=0x08;
				/* NOP NOP (size 2) */
				s->ss->tcpoptions[8]=0x01; s->ss->tcpoptions[9]=0x01;
				/* SACK permitted (size 2) */
				s->ss->tcpoptions[10]=0x04; s->ss->tcpoptions[11]=0x02;
			}
			break;

		case 9:
			/*
			 * Linux 5.x/6.x fingerprint
			 * From p0f v3: *:64:0:*:mss*20,7:mss,sok,ts,nop,ws:df,id+:0
			 * TTL=64, DF=1, Window=MSS*20, WScale=7
			 * Options: MSS, SACK, TS, NOP, WS7 (20 bytes)
			 */
			if (s->ss->minttl == 0 && s->ss->maxttl == 0) {
				scan_setttl("64");
			}
			...
			if (s->ss->mode == MODE_TCPSCAN || s->ss->mode == MODE_TCPTRACE) {
				...
				s->ss->tcpoptions[16]=0x01;
				/* WS 7 (size 3) */
				s->ss->tcpoptions[17]=0x03; s->ss->tcpoptions[18]=0x03; s->ss->tcpoptions[19]=0x07;
			}
			break;

		case 10:
			/*
			 * macOS/iOS fingerprint
			 * From p0f v3: *:64:0:*:65535,4:mss,nop,ws,nop,nop,ts,sok,eol+1:df,id+:0
			 * TTL=64, DF=1, Window=65535, WScale=4
			 * Options: MSS, NOP, WS4, NOP, NOP, TS, SACK, EOL (24 bytes)
			 */
			if (s->ss->minttl == 0 && s->ss->maxttl == 0) {
				scan_setttl("64");
			}
			...
			if (s->ss->mode == MODE_TCPSCAN || s->ss->mode == MODE_TCPTRACE) {
				...
				s->ss->tcpoptions[4]=0x01;
				/* WS 4 (size 3) */
				s->ss->tcpoptions[5]=0x03; s->ss->tcpoptions[6]=0x03; s->ss->tcpoptions[7]=0x04;
				/* NOP NOP (size 2) */
				s->ss->tcpoptions[8]=0x01; s->ss->tcpoptions[9]=0x01;
				/* Timestamp (size 10) */
				s->ss->tcpoptions[10]=0x08; s->ss->tcpoptions[11]=0x0a;
				...
				memcpy(s->ss->tcpoptions + 16, &r_tstamp, sizeof(r_tstamp));
				/* SACK permitted (size 2) */
				s->ss->tcpoptions[20]=0x04; s->ss->tcpoptions[21]=0x02;
				/* EOL + 1 byte padding (size 2) */
				s->ss->tcpoptions[22]=0x00; s->ss->tcpoptions[23]=0x00;
			}
			break;

		case 11:
			/*
			 * Android fingerprint (based on Linux 4.x kernel)
			 * Similar to Linux but with different window scale
			 * TTL=64, DF=1, Window=MSS*10, WScale=6
			 * Options: MSS, SACK, TS, NOP, WS6 (20 bytes)
			 */
			if (s->ss->minttl == 0 && s->ss->maxttl == 0) {
				scan_setttl("64");
			}
			...
			if (s->ss->mode == MODE_TCPSCAN || s->ss->mode == MODE_TCPTRACE) {
				...
				s->ss->tcpoptions[16]=0x01;
				/* WS 6 (size 3) */
				s->ss->tcpoptions[17]=0x03; s->ss->tcpoptions[18]=0x03; s->ss->tcpoptions[19]=0x06;
			}
			break;

		case 12: /* random tcp options */
		default:
