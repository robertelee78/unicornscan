 ...
 **********************************************************************/
/*
 * TLS 1.2/1.3 ClientHello payload module with SNI extension
 *
 * Generates a modern TLS ClientHello that elicits ServerHello responses
 * from TLS-enabled services. Uses the target IP address as the SNI
 * hostname since the module API does not provide hostname resolution.
 *
 * Features:
 * - TLS 1.2 record layer (compatible with TLS 1.3 servers)
 * - Modern cipher suite selection (AEAD ciphers)
 * - SNI extension with target IP
 * - supported_versions extension for TLS 1.3 negotiation
 * - signature_algorithms extension (required by many servers)
 * - supported_groups extension for ECDHE
 *
...
#define CIPHER_SUITES_LEN	sizeof(cipher_suites)
...
#define SUPPORTED_GROUPS_LEN	sizeof(supported_groups)
...
#define SIGNATURE_ALGORITHMS_LEN	sizeof(signature_algorithms)

/*
 * Supported TLS versions for supported_versions extension
 */
static const uint8_t supported_versions[] = {
	0x03, 0x04,	/* TLS 1.3	*/
	0x03, 0x03,	/* TLS 1.2	*/
	0x03, 0x02,	/* TLS 1.1	*/
	0x03, 0x01,	/* TLS 1.0	*/
};
#define SUPPORTED_VERSIONS_LEN	sizeof(supported_versions)
...
int init_module(mod_entry_t *m) {
	...
	snprintf(m->desc, sizeof(m->desc) - 1, "TLS 1.2/1.3 ClientHello with SNI");
	m->iver = 0x0103;
...
int create_payload(uint8_t **data, uint32_t *dlen, void *i) {
	 ...
	 *   Extensions
	 */
	client_hello_len = 2 + 32 + 1 + 2 + CIPHER_SUITES_LEN + 1 + 1 + 2 + ext_len;
	...
	*p++ = (CIPHER_SUITES_LEN >> 8) & 0xff;
	*p++ = CIPHER_SUITES_LEN & 0xff;

	/* Cipher Suites */
	memcpy(p, cipher_suites, CIPHER_SUITES_LEN);
	p += CIPHER_SUITES_LEN;
	...
	*p++ = EXT_SUPPORTED_GROUPS & 0xff;
	/* Extension Length */
	*p++ = ((2 + SUPPORTED_GROUPS_LEN) >> 8) & 0xff;
	*p++ = (2 + SUPPORTED_GROUPS_LEN) & 0xff;
	/* Supported Groups List Length */
	*p++ = (SUPPORTED_GROUPS_LEN >> 8) & 0xff;
	*p++ = SUPPORTED_GROUPS_LEN & 0xff;
	/* Supported Groups */
	memcpy(p, supported_groups, SUPPORTED_GROUPS_LEN);
	p += SUPPORTED_GROUPS_LEN;
	...
	*p++ = EXT_SIGNATURE_ALGORITHMS & 0xff;
	/* Extension Length */
	*p++ = ((2 + SIGNATURE_ALGORITHMS_LEN) >> 8) & 0xff;
	*p++ = (2 + SIGNATURE_ALGORITHMS_LEN) & 0xff;
	/* Signature Algorithms List Length */
	*p++ = (SIGNATURE_ALGORITHMS_LEN >> 8) & 0xff;
	*p++ = SIGNATURE_ALGORITHMS_LEN & 0xff;
	/* Signature Algorithms */
	memcpy(p, signature_algorithms, SIGNATURE_ALGORITHMS_LEN);
	p += SIGNATURE_ALGORITHMS_LEN;
	...
	*p++ = EXT_SUPPORTED_VERSIONS & 0xff;
	/* Extension Length */
	*p++ = ((1 + SUPPORTED_VERSIONS_LEN) >> 8) & 0xff;
	*p++ = (1 + SUPPORTED_VERSIONS_LEN) & 0xff;
	/* Supported Versions List Length */
	*p++ = SUPPORTED_VERSIONS_LEN & 0xff;
	/* Supported Versions */
	memcpy(p, supported_versions, SUPPORTED_VERSIONS_LEN);
	p += SUPPORTED_VERSIONS_LEN;
